...
void main(void)
{
	...
	new_radio_button = gtk_radio_button_new_with_mnemonic(NULL, "_Another RadioButton");
	gtk_grid_attach(GTK_GRID(grid), new_radio_button, 0, 1, 1, 1);

	/*
	GSList *get_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(radio_button));
	gtk_radio_button_set_group(GTK_RADIO_BUTTON(new_radio_button), get_group);
	*/

	gtk_radio_button_join_group(GTK_RADIO_BUTTON(new_radio_button), GTK_RADIO_BUTTON(radio_button));
