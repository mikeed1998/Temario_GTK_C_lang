...
gboolean actv_link_clbk(GtkLinkButton*);
...
void main(void)
{
	...
	linkButton = gtk_link_button_new_with_label("http://gnome.org", label);
	...
	/* Signals */
	g_signal_connect_swapped(linkButton, "activate-link", G_CALLBACK(actv_link_clbk), linkButton);
...
gboolean actv_link_clbk(GtkLinkButton *linkButton)
{
	// g_print("The visitate state is %d\n", gtk_link_button_get_visited(linkButton));
	// gtk_link_button_set_visited(linkButton, TRUE);
	...
	if(!gtk_link_button_get_visited(linkButton)) {
		g_print("Opening YouTube for the first time\n");
		count++;
		return FALSE;
	} else {
		g_print("The YouTube link was clicked %d times\n", ++count);
		return FALSE;
	}
