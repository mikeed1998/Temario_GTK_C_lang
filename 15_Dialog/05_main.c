...
void run2_dialog_callback(GtkWindow*);
...
void main(void)
{
	...
	g_signal_connect_swapped(button, "clicked", G_CALLBACK(run2_dialog_callback), window);
...
void run2_dialog_callback(GtkWindow *window)
{
	if(!GTK_IS_WINDOW(window))
	{
		g_print("the provided parameter is wrgn\n");
